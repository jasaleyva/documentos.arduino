...
//Pines display4Bits: 
//marron-5V, rojo-6, naranja-4, amarillo-3, verde-GND
...
FourBitDisplay fbd(7,8,1); //pines libres en simón
 ...
 // Iniciamos las resistencias PullUp internas para los botones
  ...
  void mostrar_display();//muestra en display el numero de aciertos = variable contador
  delay(1000);          // Espera 1 segundo
  }
  //Función para mostrar en display el número de aciertos mediante la variable "contador"
  void mostrar_display(){
     //int  aciertos = contador;//creamos la variable aciertos y le asignamos el valor del contador
    char buf[4];
    //dtostrf(contador,2,0, buf)... 2=tamaño del número "aciertos", 
    //0= cantidad de decimales de "aciertos"
    dtostrf(contador,2,0, buf); //Convert from float to char array
    char *val = buf;
