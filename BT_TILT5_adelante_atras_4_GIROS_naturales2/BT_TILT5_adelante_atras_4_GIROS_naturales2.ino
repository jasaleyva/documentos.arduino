...
int MAX = 100 ; //máxima veloc de una rueda, nunca superior a 255
...
//Giro Izquierda Atras
 ...
 if (pitch <-10 && roll >-10 && roll <10 ){
   int veloc = map (pitch, 0,-90, 0,MAX);
...
if (pitch <-10  && roll <-10 ){
   int veloc_r = map (roll, 0,-90, 0,MAX); // rango veloc 0-255
   int veloc_p = map (pitch, 0,-90, 0,MAX); // rango veloc 0-255
